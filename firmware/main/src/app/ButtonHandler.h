      ...
      if (!wasPressed) {
        ...
        if (isDevMode) {
          ...
          lattice::utils::Logger::logln("MAIN",
                        ...
                        lattice::utils::LogLevel::LOG_INFO);
          lattice::utils::Logger::logln("MAIN", "Restarting in 2 seconds for new role...", lattice::utils::LogLevel::LOG_INFO);
    ...
    if (btn.isPressed()) {
      if (!wasPressed) {
        ...
        if (!confirmPending) {
          ...
          lattice::utils::Logger::logln("MAIN", "Reset armed: hold again within 3s to confirm EEPROM wipe",
                        lattice::utils::LogLevel::LOG_WARN);
          redLed.blink(3, 100, 100);
        } else if (millis() < confirmDeadline) {
          confirmPending = false;
          lattice::utils::Logger::logln("MAIN", "EEPROM wipe confirmed. Clearing all...", lattice::utils::LogLevel::LOG_WARN);
          em.clearAll();
      ...
      if (confirmPending && millis() > confirmDeadline) {
        confirmPending = false;
        lattice::utils::Logger::logln("MAIN", "Reset confirmation timed out", lattice::utils::LogLevel::LOG_INFO);
      }
    }
  }
};
