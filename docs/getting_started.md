...
inputs: the **master node's own public key** and the **master node's MAC
address**, and writes out a compiled-in C++ header.

The public key is the one the master board itself prints over serial at boot
— `LATTICE_PUBKEY:` followed by 64 hex characters (see
[Step 7](#9-step-7-first-boot--provisioning)). It is the board's own
on-device identity: the key it puts in every enrollment approval it sends,
and the key other nodes encrypt to. It is **not** the `masterkey.json` file
the `lattice-hub` server generates — that is the hub *process's* own
identity, which no board holds the private half of, so a pin made from it
would never match a real master and every node would silently refuse to
enroll ([issue #126](https://github.com/superbrobenji/lattice-nodes/issues/126)).
The generator refuses `.json` input for that reason.

**For a first-time "just get it building" experience** — or for the master
board's own first build, which has to happen *before* the board can print its
key — generate the header from a throwaway random key and a placeholder MAC.
This is enough to compile and bench-test the firmware, but it will **not**
let a node trust a real master. From the `lattice-nodes` folder, run:

```bash
python3 tools/gen_master_pubkey_pin.py "$(python3 -c 'import os; print(os.urandom(32).hex())')" aa:bb:cc:dd:ee:ff
```

- The inner `python3 -c ...` produces 32 random bytes as 64 hex characters,
  in the same shape as a real `LATTICE_PUBKEY:` value.
- The generator writes the compiled header for that key and the placeholder
  MAC address (`aa:bb:cc:dd:ee:ff` — a conventional placeholder, not a real
  device's address).
...
**Before a real deployment against a real hub**, regenerate it from the
actual master board: flash and boot the master once (it can run the
throwaway-pin build — a master never checks the pin against itself), copy
its `LATTICE_PUBKEY:...` line from the serial monitor, read its MAC with
`esptool.py read_mac`, then:

```bash
python3 tools/gen_master_pubkey_pin.py LATTICE_PUBKEY:<64 hex chars> <master MAC>
```

(You can also hand the generator a saved copy of the monitor output instead
of the hex — it picks the `LATTICE_PUBKEY:` line out of it.) Rebuild, and
flash **every** node in the mesh from that build — they must all be built
against the same real master identity, or they will refuse to trust the
master's `JOIN_ACK` messages. The master keeps its keypair across normal
re-flashes (`idf.py flash`), so re-flashing it with the final build does not
change its key. What *does* change it: a factory reset (reset button,
[Step 9](#11-step-9-using-the-buttons)) or `idf.py erase-flash` — after
either, the master's key is new, every node's compiled-in pin is stale, and
you must repeat this step and re-flash all nodes.

**Do not commit `master_pubkey_pin.h`.** It's a per-deployment identifier
(the key itself is public, but it uniquely names *your* master), and it is
...
If this board is going to be your **master**, this line is also one of the
two inputs to the pin file every other node is built with — see
[Step 3](#5-step-3-generate-your-master-pubkey-pin-file). A master prints
it on every boot (it never "enrolls"), so you can always read it again by
resetting the board with the monitor open.
...
| A number with a **solid** decimal point lit (normally `0.`) | **Primary master.** This board is acting as master *and* its MAC is the `MASTER_MAC` pinned into the firmware at build time (`master_pubkey_pin.h`, [Step 3](#5-step-3-generate-your-master-pubkey-pin-file)). Masters skip the hub's node-ID assignment, so the digit is normally `0`. |
| A number with a decimal point **blinking** about twice a second (normally `0.`) | **Secondary master.** This board is acting as master but its MAC is *not* the pinned `MASTER_MAC` — the standby board in a two-master deployment (`DUAL_MASTER_MODE`, see the [dual-master section of `macos_dev_bringup.md`](macos_dev_bringup.md#dual-master-setup)). If you see this on a single-master mesh, or on a `DEV_MODE` build using the placeholder pin, this board's MAC doesn't match the pin file the firmware was built with. |
...
device's MAC address. The `master_pubkey_pin.h` compiled into *this* board is
unaffected (it's in the firmware, not stored on the device) — you do not need
to reflash the board — but you will need to re-enroll the node with your hub
afterward, as if it were brand new (it will print a fresh
`LATTICE_PUBKEY:...` line on its next boot).

**If the board you wiped is the master, that fresh key is the problem:** every
other node was built with a pin of the master's *old* key, so they will now
silently refuse its `JOIN_ACK`s. Regenerate the pin from the new
`LATTICE_PUBKEY:` line ([Step 3](#5-step-3-generate-your-master-pubkey-pin-file))
and re-flash every node. The same applies after `idf.py erase-flash` on the
master.
...
`tools/gen_master_pubkey_pin.py` command in that section (with a throwaway
random key if you don't have a master's `LATTICE_PUBKEY` yet) before
building again.
...
If the hub *has* approved it and it still never enrolls, the usual cause is a
**pin mismatch**: the node's firmware was built with a `master_pubkey_pin.h`
that doesn't hold this master's current `LATTICE_PUBKEY` — a throwaway key
from the first bench build, a pin generated from the hub's `masterkey.json`
by an older checkout, or the master having been factory-reset since the pin
was made. The node drops the approval silently in production builds.
Regenerate the pin from the master's current `LATTICE_PUBKEY:` line
([Step 3](#5-step-3-generate-your-master-pubkey-pin-file)) and re-flash.
