...
**Step B — `CONFIG_ESP_WIFI_MBEDTLS_CRYPTO=n`.** wpa_supplicant falls back to its internal crypto implementations. This firmware is pure ESP-NOW (no AP association, no WPA handshake at runtime), so that path is near-dead code either way. Removes the six selects listed above.
